...
public class DisruptorService {
    ...
    private void processBid(BiddingEvent event) {
        ...
        Mono<String> pacingMono = cacheProvider.get(String.format(RedisKeyConstants.PREFIX_AD_CAMPAIGN_PACING, selectedCampaign)).defaultIfEmpty(BiddingConstants.DEFAULT_PACING_MULTIPLIER_STRING);
        Mono<String> maxBidMono = cacheProvider.get(String.format(RedisKeyConstants.PREFIX_AD_CAMPAIGN_MAX_BID, selectedCampaign)).defaultIfEmpty("");
        Mono.zip(pacingMono, maxBidMono).subscribe(tuple -> {
            ...
            event.preAllocatedBid.nurl = String.format("http://event-tracking-service/api/v1/tracking/impression?campaignId=%s&advertiserId=%s&wp=%s", selectedCampaign, advertiserId, AdMacroConstants.MACRO_AUCTION_PRICE);
